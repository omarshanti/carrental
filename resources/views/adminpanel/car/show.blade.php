            ...
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-6">
                    ...
                    <div class="table-responsive mb-4 mt-4">
                        ...
                        <table data-order='[[ 0, "desc" ]]' id="alter_pagination" class="table table-hover" style="width:100%">
                            ...
                            <tbody>
                              @foreach($car as $cars)
                                <tr>
                                    <td>{{$cars->id}}</td>
                                    <td><img src="{{asset('assets/images/'.$cars->Vimage1)}}" width="100px" height="100px" alt=""></td>
                                    @if(LaravelLocalization::getCurrentLocale() == 'ar')
                                    <td>{{$cars->VehiclesTitle_ar}}</td>
                                    <td>{{$cars->Brands->car_ar}}</td>
                                    @else
                                    <td>{{$cars->VehiclesTitle}}</td>
                                    <td>{{$cars->Brands->BrandName}}</td>
                                    @endif
                                    <td>${{$cars->PricePerDay}}</td>
                                    @if(LaravelLocalization::getCurrentLocale() == 'ar')
                                    <td>{{__($cars->FuelType)}}</td>
                                    @else
                                    <td>{{$cars->FuelType}}</td>
                                    @endif
                                    <td>{{$cars->ModelYear}}</td>
                                    <td class="text-center" style="display: flex; margin-top: 39px;">
                                        
                                            <li style="list-style-type: none;"><a href="{{route('admin.Vehicles.edit',['id'=> $cars->id])}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle text-primary"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></a></li>
                                            <li style="list-style-type: none;"><a href="{{route('admin.Vehicles.delete',['id'=> $cars->id])}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x-circle text-danger"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg></a></li>
                        ...
                        <div class="row"><div class="col-sm-12 col-md-5"><div class="dataTables_info" id="style-3_info" role="status" aria-live="polite">Showing page {{$car->currentPage()}} of {{$car->lastPage()}}</div></div><div class="col-sm-12 col-md-7">{{$car->links()}}</div></div></div>
                    </div>
                </div>
            </div>
        </div>
