                    ...
                    <div class="widget-header">
                        <div class="row">
                            <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                <h4>{{__('MANAGE USER CONTACT US')}}</h4>
                            </div>                       
                        </div>
                    </div>
                    <br>
                        ...
                        <table data-order='[[ 0, "desc" ]]' id="alter_pagination" class="table table-hover" style="width:100%">
                            ...
                            @foreach($data as $contact)
                                <tr>
                                    <td>{{$contact->id}}</td>
                                    <td>{{$contact->name}}</td>
                                    <td><div style = "width:150px; word-wrap: break-word">{{$contact->EmailId}}</div></td>
                                    <td>{{$contact->ContactNo}}</td>
                                    <td>{{$contact->message}}</td>
                                    <td>{{$contact->created_at}}</td>
                                    <td class="text-center"  @if(LaravelLocalization::getCurrentLocale() == 'ar') style="display: flex; margin-top: 7px;margin-right: 28px;" @else style="display: flex; margin-top: 7px;margin-left: 28px;" @endif >
                                    
                                            <li style="list-style-type: none;"><a href="{{route('admin.contact.delete',['id'=> $contact->id])}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x-circle text-danger"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg></a></li>
